...
SHUTDOWN_EVENT_HOST = os.environ.get("SHUTDOWN_HOST", "127.0.0.1")
SHUTDOWN_EVENT_PORT = int(os.environ.get("SHUTDOWN_PORT", 5558))
...
def config_update_callback():
    ...
    logger.debug(f"Sending {RESET_COMMAND} command to {SHUTDOWN_EVENT_HOST}:{SHUTDOWN_EVENT_PORT}")
...
if __name__ == "__main__":
    ...
    comm_module = CommunicationModule(host=SHUTDOWN_EVENT_HOST, port=SHUTDOWN_EVENT_PORT)
    comm_module.start()
